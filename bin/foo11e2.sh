...
# 起点：foo11e2.sh（foo11e.sh + foo11f.sh の改良統合版）
# 特徴：壊れたタブ/ウィンドウの例外を try...on error で回避
#       タブタイトルの取得・アクティブ化に成功
#       /tmp/terminal_tab_debug.log で詳細確認可
        ...
        repeat with w in windows
            try
                repeat with t in tabs of w
                    ...
                    on error errMsg
                        set output to output & "(error)\n"
                    end try
                end repeat
            on error
                -- skip inaccessible window
            end try
        end repeat
    on error
        return ""
